...
public class StringNote : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        str = " Abc Def Fed Cba ";      // 앞뒤에 공백을 하나씩 주어서 값 저장

        Debug.Log(str);
        Debug.Log(str.Length);          // 문자열 길이 구하기
        Debug.Log(str[5]);              // 'D' 문자 출력하기
        ...
        Debug.Log(str.Substring(5, 3)); // 5번 인덱스부터 3글자 가져오기(문자열 일부 가져오기)
        Debug.Log(str.Substring(5));    // 5번 인덱스부터 끝까지 가져오기
        Debug.Log(str.Remove(5, 3));    // 5번 인덱스로부터 3글자 삭제
